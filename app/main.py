...
def descargarAudio(enlace):
    try:
        videoProcesado = YouTube(enlace)
        audio = videoProcesado.streams.filter(res="720p").first()

        # Cambiar el nombre del archivo descargado para asegurar la extensión correcta
        audio_extension = audio.subtype.split(";")[0]
        audio_path = os.path.join(descargas_path, audio.title + "." + audio_extension)

        # Descargar el archivo de audio
        audio.download(output_path=descargas_path)

        # Convertir el archivo de audio a MP3
        mp3_path = os.path.join(descargas_path, audio.title + ".mp3")
        audio_clip = VideoFileClip(audio_path)
        audio_clip.audio.write_audiofile(mp3_path)
        audio_clip.close()

        # Eliminar el archivo original (.mp4)
        os.remove(audio_path)

        MessageBox.showinfo('Correcto', f'Audio descargado en formato MP3')
    except Exception as e:
        MessageBox.showinfo('Error', f'Error desconocido: {e}')
